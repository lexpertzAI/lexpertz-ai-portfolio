      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-20 left-0 right-0 bg-brand-black border-b border-white/10 p-6 flex flex-col gap-6 md:hidden backdrop-blur-2xl"
          >
            {navLinks.map((link) => (
              <Link
                key={link.name}
            ...
            <button className="w-full bg-cyan-500 text-black font-bold py-4 rounded-xl">
              Start Project
            </button>
          </motion.div>
        )}
